    ...
    def matchGstMultiple(self):
        ...
        if result:
            ...
            for gst_mci in self.guiGstPreSelTview._mci_list:
                ...
                if first_entity:
                    self.parent.initView()

                # self.parent.view.model().sourceModel().loadLayer()
                self.parent._gis_layer.commitChanges()

                self.parent._gis_layer.data_provider.reloadData()
                self.parent._gis_layer.triggerRepaint()

                self.parent._gis_layer.data_provider.dataChanged.emit()

                # self.parent.view.model().sourceModel().modelChanged.emit()
